"""Transforms compartilhadas usando funções puras e a API v2 do PyTorch (
Paradigma Funcional - DRY)."""
    ...
    return v2.Compose(
        _base_pipeline(image_size) + [
            ...
            *_to_tensor_pipeline(),
            ...
            v2.RandomErasing(
                p=0.3, scale=(0.02, 0.08), ratio=(0.3, 3.3), value='random',
            ),
        ],
    )
...
def build_eval_transform(image_size: int = IMAGE_SIZE) -> v2.Compose:
    return v2.Compose(
        _base_pipeline(image_size) + _to_tensor_pipeline() + [
            v2.Normalize(mean=IMAGENET_MEAN, std=IMAGENET_STD),
        ],
    )
...
_PERTURBATION_RECIPES: List[Tuple[str, str, object, bool]] = [
    (
        ...
        v2.Lambda(add_gaussian_noise(std=0.05)), True,
    ),
    (
        "Noise_Moderada", "Noise Moderado\n(std=0.15)",
        v2.Lambda(add_gaussian_noise(std=0.15)), True,
    ),
    (
        "Noise_Extrema", "Noise Extremo\n(std=0.30)",
        v2.Lambda(add_gaussian_noise(std=0.30)), True,
    ),
    (
        "Blur_Leve", "Blur Leve\n(k=3, s=1.0)",
        v2.GaussianBlur(kernel_size=3, sigma=1.0), False,
    ),
    (
        "Blur_Moderada", "Blur Moderado\n(k=5, s=2.0)",
        v2.GaussianBlur(kernel_size=5, sigma=2.0), False,
    ),
    (
        "Blur_Extrema", "Blur Extremo\n(k=9, s=4.0)",
        v2.GaussianBlur(kernel_size=9, sigma=4.0), False,
    ),
    (
        "Contrast_Leve", "Contrast Leve\n(60%)", v2.Lambda(apply_contrast(0.6)),
        False,
    ),
    (
        "Contrast_Moderada", "Contrast Moderado\n(30%)",
        v2.Lambda(apply_contrast(0.3)), False,
    ),
    (
        "Contrast_Extrema", "Contrast Extremo\n(10%)",
        v2.Lambda(apply_contrast(0.1)), False,
    ),
]


def _build_perturbation_dict(
        visualize: bool = False, image_size: int = IMAGE_SIZE,
) -> Dict[str, v2.Compose]:
    """Constrói dinamicamente os dicionários de robustez evitando duplicar as
    9 operações."""
    base = _base_pipeline(image_size)
    tensor_conv = _to_tensor_pipeline()
    norm = [] if visualize else [
        v2.Normalize(mean=IMAGENET_MEAN, std=IMAGENET_STD),
    ]
...
def build_perturbation_transforms(image_size: int = IMAGE_SIZE) -> Dict[
    str, v2.Compose]:
    """Transforms para avaliação métrica de robustez (com normalização
    ImageNet)."""
...
def build_visualization_perturbation_transforms(image_size: int = IMAGE_SIZE)\
        -> \
Dict[str, v2.Compose]:
    """Transforms para visualização humana (sem normalização ImageNet,
    com quebra de linha)."""

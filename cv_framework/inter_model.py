...
def calculate_inter_model_metrics(results: List[Dict]) -> Tuple[
    np.ndarray, List[str], dict, np.ndarray]:
    """Calcula a matriz de acordo, teste de McNemar e falhas genuínas sem
    gerar gráficos."""
    if len(results) < 2:
    ...
    for mod2 in model_names:
        ...
        contingency_table = [
            [
                np.sum((p1 == true_labels) & (p2 == true_labels)),
                np.sum((p1 == true_labels) & (p2 != true_labels)),
            ],
            [
                np.sum((p1 != true_labels) & (p2 == true_labels)),
        ...
        mcnemar_results[f"{top_model}_vs_{mod2}"] = mcnemar(
            contingency_table, exact=True,
        ).pvalue

    # 3. Falhas Genuínas
    all_preds = np.array([preds_dict[m] for m in model_names])
    erros_totais_idx = np.where(np.sum(all_preds == true_labels, axis=0) == 0)[
        0]
